...
import android.os.ParcelUuid // Import ParcelUuid
...
// 1. Define the UUID and a ParcelUuid object for the filter
private val TARGET_SERVICE_UUID: UUID = UUID.fromString("A495FF20-C5B5-4B44-B512-1370F02D74DE")
private val TARGET_SERVICE_PARCEL_UUID: ParcelUuid = ParcelUuid(TARGET_SERVICE_UUID)
...
class MainActivity : AppCompatActivity() {
    ...
    // 2. Define the ScanFilter list using the TARGET_SERVICE_PARCEL_UUID
    private val scanFilters: List<ScanFilter> = listOf(
        ScanFilter.Builder()
            .setServiceUuid(TARGET_SERVICE_PARCEL_UUID)
            .build()
    )
    ...
    private fun startBleScan() {
        if (!hasRequiredBluetoothPermissions()) {
            ...
            // 3. Pass the scanFilters list to the startScan function
    ...
    private val scanCallback = object : ScanCallback() {
        override fun onScanResult(callbackType: Int, result: ScanResult) {
            // Devices here are already filtered by TARGET_SERVICE_UUID
